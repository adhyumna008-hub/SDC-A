...
export const AdminDashboard: React.FC = () => {
  ...
  const [editingHistoryEvent, setEditingHistoryEvent] = useState<EventItem | null>(null);
  ...
  // Winner Edit Form State
  const [winner1st, setWinner1st] = useState('');
  const [winner2nd, setWinner2nd] = useState('');
  const [winner3rd, setWinner3rd] = useState('');

  // Add Past Event Modal State
  const [showPastEventModal, setShowPastEventModal] = useState(false);
  const [pastTitle, setPastTitle] = useState('');
  const [pastCategory, setPastCategory] = useState<'workshop' | 'hackathon' | 'speaker' | 'contest'>('hackathon');
  const [pastDescription, setPastDescription] = useState('');
  const [pastDate, setPastDate] = useState('Oct 15, 2024');
  const [pastLocation, setPastLocation] = useState('Main Auditorium & Lab 304');
  const [pastAttendees, setPastAttendees] = useState(140);
  const [pastCover, setPastCover] = useState('https://images.unsplash.com/photo-1531482615713-2afd69097998?auto=format&fit=crop&w=1200&q=80');
  const [pastGalleryUrls, setPastGalleryUrls] = useState('');
  const [pastWinner1Team, setPastWinner1Team] = useState('');
  const [pastWinner1Members, setPastWinner1Members] = useState('');
  const [pastWinner1Project, setPastWinner1Project] = useState('');
  const [pastWinner2Team, setPastWinner2Team] = useState('');
  const [pastWinner2Members, setPastWinner2Members] = useState('');
  const [pastWinner3Team, setPastWinner3Team] = useState('');
  const [pastWinner3Members, setPastWinner3Members] = useState('');
  ...
  const handleCreatePastEvent = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!pastTitle.trim() || !pastDescription.trim()) return;

    // Parse gallery images (split by new line or comma)
    const galleryImages = pastGalleryUrls
      .split(/[\n,]/)
      .map(url => url.trim())
      .filter(url => url.startsWith('http://') || url.startsWith('https://'));

    if (galleryImages.length === 0 && pastCover) {
      galleryImages.push(pastCover);
    }

    const winners: EventWinner[] = [];
    if (pastWinner1Team.trim()) {
      winners.push({
        position: '1st Place',
        teamOrName: pastWinner1Team.trim(),
        members: pastWinner1Members.trim() || undefined,
        projectTitle: pastWinner1Project.trim() || undefined
      });
    }
    if (pastWinner2Team.trim()) {
      winners.push({
        position: '2nd Place',
        teamOrName: pastWinner2Team.trim(),
        members: pastWinner2Members.trim() || undefined
      });
    }
    if (pastWinner3Team.trim()) {
      winners.push({
        position: '3rd Place',
        teamOrName: pastWinner3Team.trim(),
        members: pastWinner3Members.trim() || undefined
      });
    }
    ...
    try {
      const created = await createEventService({
        ...
        description: pastDescription.trim(),
        status: 'completed',
        image: pastCover.trim() || 'https://images.unsplash.com/photo-1531482615713-2afd69097998?auto=format&fit=crop&w=1200&q=80',
        date: pastDate.trim(),
        location: pastLocation.trim(),
        registration_start_time: new Date('2024-01-01').toISOString(),
        registration_end_time: new Date('2024-10-14').toISOString(),
        is_inter_college: true,
        min_team_size: 1,
        max_team_size: 4,
        max_seats: Number(pastAttendees) || 150,
        registered_count: Number(pastAttendees) || 120,
        attendance_count: Number(pastAttendees) || 120,
      ...
      setPastGalleryUrls('');
      setPastWinner1Team('');
      setPastWinner1Members('');
      setPastWinner1Project('');
      setPastWinner2Team('');
      setPastWinner2Members('');
      setPastWinner3Team('');
      setPastWinner3Members('');
    } catch (err) {
      console.error('Error creating past event recap', err);
    }
  };
  ...
  const handleSaveWinners = async () => {
    ...
    const winners = [
      { position: '1st Place' as const, teamOrName: winner1st || 'Team Alpha' },
      { position: '2nd Place' as const, teamOrName: winner2nd || 'Team Beta' },
      { position: '3rd Place' as const, teamOrName: winner3rd || 'Team Gamma' }
    ].filter(w => w.teamOrName);

    await updateEventService(editingHistoryEvent.id, { winners });
    setEvents(events.map(e => e.id === editingHistoryEvent.id ? { ...e, winners } : e));
    ...
    <main className="flex-1 flex flex-col min-h-screen bg-background relative pt-20">
      ...
      <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center px-container-padding py-4 border-b border-outline-variant/10 bg-surface/80 backdrop-blur-md z-10 sticky top-16">
        ...
        <div className="flex flex-wrap items-center gap-2.5 mt-3 sm:mt-0">
          ...
          <button
            onClick={() => setShowPastEventModal(true)}
          ...
          >
            <span className="material-symbols-outlined text-sm text-[#A855F7]">photo_library</span>
            <span>Recap Past Event</span>
          </button>
      ...
      <div className="p-container-padding space-y-8 max-w-7xl mx-auto w-full">
        ...
        {/* Bottom Row: Past Event Ledger & Winners Editor */}
        <section className="bg-surface-gray border border-outline-variant/10 rounded-2xl overflow-hidden mb-12">
          <div className="p-6 border-b border-outline-variant/10 flex justify-between items-center bg-surface-gray/50">
            <div>
              <h2 className="font-headline-lg text-lg text-white font-bold flex items-center gap-2">
                <span className="material-symbols-outlined text-neon-purple">history</span>
                Event History Ledger & Winner Standings
              </h2>
              <p className="text-xs font-code-sm text-on-surface-variant mt-1">Manage completed events, photo albums, and winner badges for certificate verification.</p>
            </div>

            <button
              onClick={() => setShowPastEventModal(true)}
              className="px-3.5 py-2 rounded-xl bg-amber-400/20 text-amber-300 border border-amber-400/40 hover:bg-amber-400 hover:text-deep-black font-code-sm text-xs font-bold transition-all flex items-center gap-1.5 shadow-[0_0_10px_rgba(251,191,36,0.2)] shrink-0"
            >
              <span className="material-symbols-outlined text-sm">add_photo_alternate</span>
              <span>Recap Past Event (Album & Winners)</span>
            </button>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse font-code-sm text-xs">
              <thead>
                <tr className="bg-surface-container-low border-b border-outline-variant/10 font-label-caps text-on-surface-variant">
                  <th className="p-4 font-normal tracking-wider">EVENT ID</th>
                  <th className="p-4 font-normal tracking-wider">EVENT NAME & DATE</th>
                  <th className="p-4 font-normal tracking-wider">WINNER STANDINGS</th>
                  <th className="p-4 font-normal tracking-wider text-right">ACTIONS</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-outline-variant/10">
                {events.filter(e => e.status === 'completed').map((evt) => (
                  <tr key={evt.id} className="hover:bg-surface-container/50 transition-colors">
                    <td className="p-4 font-mono text-outline">{evt.id}</td>
                    <td className="p-4">
                      <div className="font-bold text-white text-sm">{evt.title}</div>
                      <div className="text-on-surface-variant text-[11px]">{evt.date}</div>
                    </td>
                    <td className="p-4">
                      {evt.winners && evt.winners.length > 0 ? (
                        <div className="flex items-center gap-2">
                          <span className="bg-success-glow/20 text-success-glow border border-success-glow/30 px-2 py-0.5 rounded font-bold">
                            🏆 {evt.winners[0].teamOrName} (1st)
                          </span>
                        </div>
                      ) : (
                        <span className="text-outline">No winners recorded</span>
                      )}
                    </td>
                    <td className="p-4 text-right">
                      <button
                        onClick={() => {
                          setEditingHistoryEvent(evt);
                          setWinner1st(evt.winners?.[0]?.teamOrName || '');
                          setWinner2nd(evt.winners?.[1]?.teamOrName || '');
                          setWinner3rd(evt.winners?.[2]?.teamOrName || '');
                        }}
                        className="px-3 py-1 bg-neon-purple/20 text-neon-purple border border-neon-purple/40 rounded hover:bg-neon-purple hover:text-white transition-all"
                      >
                        Edit Winners
                      </button>
                    </td>
      ...
      {editingHistoryEvent && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-deep-black/80 backdrop-blur-md animate-fadeIn">
          <div className="bg-surface-gray border border-outline-variant/30 rounded-2xl max-w-md w-full p-6 relative shadow-2xl space-y-4">
            <h3 className="font-headline-lg text-lg font-bold text-white">Record Event Winners ({editingHistoryEvent.title})</h3>
            <div className="space-y-3 text-xs font-body-md">
              <div>
                <label className="block text-tertiary font-code-sm mb-1">🥇 1st Place Team / Winner Name</label>
                <input
                  type="text"
                  value={winner1st}
                  onChange={(e) => setWinner1st(e.target.value)}
                  className="w-full bg-surface-container border border-outline-variant/30 rounded-lg p-2 text-white"
                />
              </div>
              <div>
                <label className="block text-electric-cyan font-code-sm mb-1">🥈 2nd Place Team / Winner Name</label>
                <input
                  type="text"
                  value={winner2nd}
              ...
              <div>
                <label className="block text-neon-purple font-code-sm mb-1">🥉 3rd Place Team / Winner Name</label>
                <input
                  type="text"
                  value={winner3rd}
            ...
            <div className="flex justify-end gap-3 pt-2">
              <button onClick={() => setEditingHistoryEvent(null)} className="px-4 py-2 text-on-surface-variant font-code-sm text-xs">
                Cancel
              </button>
              <button onClick={handleSaveWinners} className="px-5 py-2 bg-neon-purple text-white rounded font-label-caps text-xs uppercase font-bold">
                Save Winners
              </button>
            </div>
          </div>
      ...
      {/* Add Past Event with Photo Album & Winners Modal */}
      {showPastEventModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-5 bg-deep-black/85 backdrop-blur-xl animate-fadeIn">
          <div className="bg-[#0b1326] border border-white/20 rounded-3xl max-w-2xl w-full p-6 sm:p-8 relative shadow-2xl overflow-y-auto max-h-[92vh] space-y-5">
            
            {/* Header */}
            <div className="flex justify-between items-start border-b border-white/10 pb-4">
              <div>
                <span className="bg-amber-400/20 text-amber-300 text-[10px] font-label-caps uppercase px-2.5 py-0.5 rounded-lg border border-amber-400/40 font-bold">
                  Event History Management
                </span>
                <h3 className="font-headline-lg text-xl sm:text-2xl font-bold text-white mt-1 flex items-center gap-2">
                  <span className="material-symbols-outlined text-amber-400">photo_library</span>
                  Recap Past Event with Photo Album
                </h3>
                <p className="text-xs font-code-sm text-on-surface-variant mt-0.5">
                  Publish completed event recap, photo gallery album, and winner standings to the historical ledger.
                </p>
              </div>
              <button
            ...
            <form onSubmit={handleCreatePastEvent} className="space-y-4 font-body-md text-xs">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="sm:col-span-2">
                  ...
                  <input
                    ...
                    placeholder="e.g. SDC Winter Hackfest 2024"
                    className="w-full bg-surface-container border border-outline-variant/30 rounded-xl p-3 text-white focus:border-amber-400 outline-none"
                  />
                </div>
                ...
                <div>
                  ...
                  <select
                    value={pastCategory}
                    onChange={(e) => setPastCategory(e.target.value as any)}
                    className="w-full bg-surface-container border border-outline-variant/30 rounded-xl p-3 text-white focus:border-amber-400 outline-none"
                  >
                    <option value="hackathon">Hackathon</option>
                    <option value="workshop">Workshop</option>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                  <label className="block text-on-surface-variant font-code-sm mb-1">Date Conducted</label>
                  <input
                    type="text"
                    value={pastDate}
                ...
                <div>
                  <label className="block text-on-surface-variant font-code-sm mb-1">Location / Venue</label>
                  <input
                    type="text"
                    value={pastLocation}
                ...
                <div>
                  <label className="block text-on-surface-variant font-code-sm mb-1">Total Attendees Verified</label>
                  <input
                    type="number"
                    value={pastAttendees}
                    onChange={(e) => setPastAttendees(Number(e.target.value))}
                    className="w-full bg-surface-container border border-outline-variant/30 rounded-xl p-3 text-white focus:border-amber-400 outline-none"
                  />
                </div>
              </div>
              ...
              {/* Description */}
              <div>
                <label className="block text-on-surface-variant font-code-sm mb-1">Recap & Key Highlights *</label>
                <textarea
                  required
                  rows={3}
                  value={pastDescription}
                  onChange={(e) => setPastDescription(e.target.value)}
                  placeholder="Summarize the event, standout projects built, and student participation..."
                  className="w-full bg-surface-container border border-outline-variant/30 rounded-xl p-3 text-white focus:border-amber-400 outline-none resize-none"
                />
              </div>

              {/* Cover Image */}
              <div>
                <label className="block text-on-surface-variant font-code-sm mb-1">Cover Image URL</label>
                <input
                  type="url"
                  value={pastCover}
                  onChange={(e) => setPastCover(e.target.value)}
                  className="w-full bg-surface-container border border-outline-variant/30 rounded-xl p-3 text-white font-mono text-xs focus:border-amber-400 outline-none"
                />
              </div>

              {/* Photo Album Gallery URLs */}
              <div className="bg-white/[0.03] border border-white/10 rounded-2xl p-4 space-y-3">
                <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-2">
                  <div>
                    <label className="block text-white font-bold font-code-sm">
                      📸 Event Photo Album (Multiple URLs)
                    </label>
                    <span className="text-[11px] text-on-surface-variant">
                      Paste image URLs separated by new lines or commas for the interactive lightbox album.
                    </span>
                  </div>
                  <button
                    type="button"
                    ...
                    className="text-[10px] font-mono px-2.5 py-1 rounded-lg bg-white/10 hover:bg-white/20 text-electric-cyan border border-electric-cyan/30 transition-colors shrink-0"
                  >
                    + Auto-Fill Sample Gallery (4 Photos)
                  </button>
                </div>

                <textarea
                  rows={3}
                  value={pastGalleryUrls}
                  onChange={(e) => setPastGalleryUrls(e.target.value)}
                  placeholder="https://example.com/photo1.jpg&#10;https://example.com/photo2.jpg&#10;https://example.com/photo3.jpg"
                  className="w-full bg-surface-container border border-outline-variant/30 rounded-xl p-3 text-white font-mono text-xs focus:border-amber-400 outline-none resize-none"
                />

                {/* Live Preview Thumbnails */}
                {pastGalleryUrls.split(/[\n,]/).filter(u => u.trim().startsWith('http')).length > 0 && (
                  <div className="flex items-center gap-2 overflow-x-auto pt-2">
                    <span className="text-[10px] text-on-surface-variant font-code-sm uppercase shrink-0">Preview:</span>
                    {pastGalleryUrls
                      .split(/[\n,]/)
                      .map(u => u.trim())
                      ...
                      .map((url, i) => (
                        <img
                          ...
                          src={url}
                          alt="preview"
                          className="w-14 h-11 object-cover rounded-lg border border-white/20 shrink-0"
              ...
              {/* Winner Standings Inputs */}
              <div className="bg-white/[0.03] border border-white/10 rounded-2xl p-4 space-y-3.5">
                <div className="text-white font-bold font-code-sm flex items-center gap-1.5">
                  <span className="material-symbols-outlined text-amber-400 text-sm">emoji_events</span>
                  <span>Winner Standings (Team & Student Names)</span>
                </div>

                {/* 1st Place */}
                <div className="space-y-1.5 p-3 rounded-xl bg-amber-400/5 border border-amber-400/20">
                  <span className="text-amber-300 font-bold text-[11px] block">🥇 1st Place (Winner)</span>
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                    <input
                      type="text"
                      value={pastWinner1Team}
                      onChange={(e) => setPastWinner1Team(e.target.value)}
                      placeholder="Team Name (e.g. NeuralX)"
                      className="bg-surface-container border border-outline-variant/30 rounded-lg p-2 text-white text-xs"
                    />
                    <input
                      type="text"
                      value={pastWinner1Members}
                      onChange={(e) => setPastWinner1Members(e.target.value)}
                      placeholder="Student Names (e.g. Rahul, Priya)"
                      className="bg-surface-container border border-outline-variant/30 rounded-lg p-2 text-white text-xs"
                    />
                    <input
                      type="text"
                      value={pastWinner1Project}
                      onChange={(e) => setPastWinner1Project(e.target.value)}
                      placeholder="Project Title (e.g. Autonomous Vision)"
                      className="bg-surface-container border border-outline-variant/30 rounded-lg p-2 text-white text-xs"
                    />
                  </div>
                </div>
                ...
                <div className="space-y-1.5 p-3 rounded-xl bg-white/[0.02] border border-white/10">
                  <span className="text-electric-cyan font-bold text-[11px] block">🥈 2nd Place (Runner Up)</span>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    <input
                      type="text"
                      value={pastWinner2Team}
                      onChange={(e) => setPastWinner2Team(e.target.value)}
                      placeholder="Team Name (e.g. CloudPioneers)"
                      className="bg-surface-container border border-outline-variant/30 rounded-lg p-2 text-white text-xs"
                    />
                    <input
                      type="text"
                      value={pastWinner2Members}
                      onChange={(e) => setPastWinner2Members(e.target.value)}
                      placeholder="Student Names (e.g. Vikram, Sneha)"
                      className="bg-surface-container border border-outline-variant/30 rounded-lg p-2 text-white text-xs"
                    />
                  </div>
                </div>
                ...
                {/* 3rd Place */}
                <div className="space-y-1.5 p-3 rounded-xl bg-white/[0.02] border border-white/10">
                  <span className="text-neon-purple font-bold text-[11px] block">🥉 3rd Place</span>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    <input
                      type="text"
                      value={pastWinner3Team}
                      onChange={(e) => setPastWinner3Team(e.target.value)}
                      placeholder="Team Name (e.g. ByteForce)"
                      className="bg-surface-container border border-outline-variant/30 rounded-lg p-2 text-white text-xs"
                    />
                    <input
                      type="text"
                      value={pastWinner3Members}
                      onChange={(e) => setPastWinner3Members(e.target.value)}
                      placeholder="Student Names (e.g. Harish, Divya)"
                      className="bg-surface-container border border-outline-variant/30 rounded-lg p-2 text-white text-xs"
                    />
                  </div>
                </div>
              ...
              <div className="flex justify-end gap-3 pt-3 border-t border-white/10">
                <button
                  type="button"
                  onClick={() => setShowPastEventModal(false)}
                  className="px-4 py-2 rounded-xl text-on-surface-variant hover:text-white font-code-sm text-xs"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="px-6 py-2.5 bg-gradient-to-r from-amber-400 to-amber-500 hover:from-amber-300 hover:to-amber-400 text-deep-black font-label-caps text-xs uppercase font-extrabold rounded-xl shadow-[0_0_20px_rgba(251,191,36,0.4)] transition-all flex items-center gap-2"
                >
                  <span className="material-symbols-outlined text-sm">publish</span>
                  <span>Publish Past Event Recap to Ledger</span>
                </button>
              </div>
            </form>
          </div>
